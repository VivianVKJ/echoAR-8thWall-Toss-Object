  ...
  init() {
    ...
    echo.findObject(true, function(data) {
      echo.models.forEach(function(value, key) {
        console.log("from vaule " + value.hologram.components["gltf-model"].attrValue)
        models.push(value.hologram.components["gltf-model"].attrValue)
    ...
    this.el.sceneEl.addEventListener('touchstart', (event) => {
      ...
      tomato.setAttribute('scale', '0.5 0.5 0.5')
      tomato.setAttribute('gltf-model', models[0])

      // Choose a random rotation offset for some variation
      const randomRotation = {x: -45 + Math.random() * 30, y: Math.random() * 360, z: 0}
      ...
      splat.setAttribute('gltf-model', models[0])
      splat.setAttribute('scale', '0.25 0.25 0.25')

      // const splat =
      //create splat in load-model.js
      ...
      tomato.addEventListener('collide', (e) => {
        ...
        // After 5 seconds, shrink the splat away and delete it
        setTimeout(() => {
          splatBase.setAttribute('animation__scale', {
            property: 'scale',
            from: '3 0.1 3',
